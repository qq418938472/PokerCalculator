class SetCardPower {
    ...
    public static ins: SetCardPower = new SetCardPower;

    public initCardPower(cards: Array<CardObject>) {
        this.changeCardsCode(cards);
        return this.setCardPower();
    }

    private changeCardsCode(cards: Array<CardObject>) {
        this.cardCodes[0] = 0;
        this.cardCodes[1] = 0;
        this.cardCodes[2] = 0;
        this.cardCodes[3] = 0;
        for (var i = 0; i < cards.length; i++) {
            this.cardCodes[cards[i].color - 1] |= (0b1 << (cards[i].value - 2));
        }
    }
        ...
        if (this.table.nRanksTable[rankmask] >= 5) {
            ...
            if (sranks >= 0) {
                ...
                if (strval > 0) {
                    ...
                    var mapCodes = strval == 3 ? 0b1000000001111 : this.table.topFiveRanksTable[(1 << strval + 1) - 1];
